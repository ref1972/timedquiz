...
- **One question on screen at a time**, with a free-text answer box and a
  submit button.
- **25-second limit per question.** If the player has not submitted when the
...
grant a fresh 25 seconds.

The rules:

1. When the server delivers question N, it records `served_at` and derives a
   deadline from it.
2. The client receives the authoritative deadline, never a flat 25000. A refresh therefore
...
- **The questions will leak.** A fixed order across a multi-day window
  guarantees it. The 25-second clock is the actual defense — a player who has
...
- **Practice round** — two or three unscored questions before Q1, so no player
  burns a real question discovering how the 25-second window feels.
...
4. Does the 25-second clock start when the question renders, or after a
